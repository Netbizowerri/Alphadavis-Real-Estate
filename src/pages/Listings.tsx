...
export default function Listings() {
  ...
  const filteredProperties = properties.filter(p => {
    const matchesSearch = (p.title || '').toLowerCase().includes(searchQuery.toLowerCase()) || 
                         (p.neighborhood || p.city || '').toLowerCase().includes(searchQuery.toLowerCase());
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredProperties.map((property, idx) => (
              <motion.div
                ...
                className="group cursor-pointer bg-brand-deep/5 border border-brand-deep/10 p-5 rounded-[3rem] border-brand-accent/5 hover:border-brand-accent/20 transition-all duration-500 shadow-xl"
              >
                ...
                <div className="space-y-3 px-2">
                  ...
                  <p className="font-display text-brand-deep/60 flex items-center gap-3 font-bold uppercase text-xs tracking-widest">
                    <MapPin size={18} className="text-brand-accent" /> {property.neighborhood || property.city}
                  </p>
                ...
                {isConsulting ? (
                  <div className="p-8 md:p-12">
                    ...
                    <div className="max-w-2xl mx-auto">
                      <ContactForm 
                        initialMessage={`I am interested in "${selectedProperty.title}" located in ${selectedProperty.neighborhood || selectedProperty.city}. I would like more information and a private consultation.`}
                  ...
                  <>
                    <div className="relative w-full h-[350px] md:h-[500px]">
                      <img 
                        src={selectedProperty.coverImageUrl} 
                      ...
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                        <div className="space-y-2">
                          <p className="font-display flex items-center gap-2 text-white/60 font-bold uppercase text-sm tracking-widest italic">
                            <MapPin size={18} className="text-brand-accent" /> {selectedProperty.neighborhood || selectedProperty.city}
                          </p>
